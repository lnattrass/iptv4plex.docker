...
def discover(tunerLimit=6,tunerNumber=""):
  S_HOST=request.headers.get('host')
	discoverData = {
		...
		'BaseURL': S_HOST if tunerNumber == "" else S_HOST + '/' + tunerNumber,
		'LineupURL': '%s/lineup.json' % (S_HOST if tunerNumber == "" else S_HOST + '/' + tunerNumber)
	}
...
def lineup(tuner='0'):
  S_HOST=request.headers.get('host')
	...
	for c in chan_map[tuner]:
		template = "{0}/auto/v{1}"
		url = template.format(S_HOST, chan_map[tuner][c].channum)
